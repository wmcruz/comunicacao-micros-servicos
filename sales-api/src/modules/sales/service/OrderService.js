...
class OrderService {
    async createOrder(req) {
        try {
            ...
            await this.validateProductStock(order, authorizathion);
    ...
    async validateProductStock(order, token) {
        let stockIsOk = await ProductClient.checkProductStock(order.products, token);
        if (stockIsOk) {
    ...
    async findByProductId(req) {
        try {
            ...
            if (!orders) {
                throw new OrderException(BAD_REQUEST, "The orders were found.");
            }
